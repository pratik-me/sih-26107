...
export default function LandingPage() {
  ...
  const handleSearchSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (searchQuery.trim()) {
      router.push(`/chat?q=${encodeURIComponent(searchQuery.trim())}&role=${currentMode}`);
    }
  };
  ...
  return (
    <div className="flex flex-col min-h-full bg-ashoka-pattern">
      ...
      <section className="relative pt-12 pb-20 px-4 sm:px-6 max-w-7xl mx-auto w-full text-center">
        {/* Top badge */}
        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-blue-50 dark:bg-blue-950/60 border border-blue-200 dark:border-blue-800 text-blue-800 dark:text-blue-300 text-xs font-semibold mb-6 shadow-sm">
          <ShieldCheck className="w-4 h-4 text-blue-600 dark:text-blue-400" />
          <span>Government AI Decision-Support Platform • 100% Grounded Citations</span>
        </div>
        ...
        <div className="mt-6 max-w-3xl mx-auto text-left">
          <span className="text-xs font-semibold text-slate-500 block mb-2">Suggested queries:</span>
      ...
      <section className="py-16 px-4 sm:px-6 max-w-7xl mx-auto w-full">
        <div className="text-center max-w-2xl mx-auto mb-12">
          ...
          <p className="mt-2 text-sm text-slate-600 dark:text-slate-400">
            Strict adherence to <span className="font-semibold text-blue-700 dark:text-blue-400">"Retrieve First → Reason Second → Cite Everything"</span>
          </p>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-5 gap-4">
          {[
            { step: '1', title: 'Ask Query', desc: 'Query in English, Hindi, or any of 22 Scheduled Indian Languages.', icon: Search },
            { step: '2', title: 'Retrieve', desc: 'Hybrid BM25 + Vector semantic search across BIS repository.', icon: BookOpen },
            { step: '3', title: 'Verify', desc: 'Cross-encoder reranking & source freshness verification.', icon: ShieldCheck },
            { step: '4', title: 'Explain', desc: 'Clear plain-language guidance distinguished from statutory clauses.', icon: FileCheck2 },
            { step: '5', title: 'Cite', desc: 'Every claim traceable to standard number, clause, page, and link.', icon: Scale }
          ].map(item => {
        ...
        <div className="max-w-5xl mx-auto flex flex-col md:flex-row items-center justify-between gap-8">
          <div className="space-y-2">
            ...
            <p className="text-xs text-slate-400 max-w-xl leading-relaxed">
              BIS IntelliGuide never invents Indian Standard numbers, test clauses, or lab recognition statuses. If official evidence is not available in the database, the system will explicitly state that the requirement cannot be verified.
            </p>
          </div>
          <Link
            href="/chat"
