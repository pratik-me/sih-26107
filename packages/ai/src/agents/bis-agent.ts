  ...
  async execute(query: string, preferredLanguage?: IndianLanguage): Promise<AgentExecutionResponse> {
    ...
    if (this.toolsHandler) {
      if (intent === QueryIntent.FIND_STANDARD) {
        ...
        retrievedEvidence = res.evidence;
        workflowOffers.push(
          { type: 'VERIFY_ISI', label: 'Check CML Number on BIS Care', actionPayload: { query: translatedText } }
        );
      } else {
    ...
    if (intent === QueryIntent.FIND_STANDARD || intent === QueryIntent.CERTIFICATION_GUIDANCE) {
      workflowOffers.push({
